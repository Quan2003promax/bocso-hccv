...
<head>
  ...
  <style>
    ...
    /* ===== HERO ===== */
    .hero-section {
      background-image: linear-gradient(115deg, rgba(6, 28, 61, .85), rgba(6, 28, 61, .35)), url('https://images.unsplash.com/photo-1519677100203-a0e668c92439?q=80&w=1920&auto=format&fit=crop');
...
<body>
        ...
        <div class="col-lg-7 text-start">
          ...
          <div class="hero-search mt-3">
            <form action="{{ route('home') }}" method="GET" class="search-box" id="queue-search-form">
              <input type="text" name="search" id="queue-search-input" placeholder="Vui lòng nhập Số thứ tự" value="{{ $searchQuery ?? '' }}" required>
          ...
          <form action="{{ route('service.register') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-3">
                ...
                <input type="number" class="form-control @error('birth_year') is-invalid @enderror" id="birth_year" name="birth_year" value="{{ old('birth_year') }}" min="1900" max="{{ date('Y') + 1 }}" required>
            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control @error('identity_number') is-invalid @enderror" id="identity_number" name="identity_number" value="{{ old('identity_number') }}" required>
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-3">
                ...
                <input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone') }}" placeholder="0123456789" required>
            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <input type="file" class="form-control @error('document_file') is-invalid @enderror" id="document_file" name="document_file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif">
                <div class="form-text">
  ...
  <script>
    ...
    // Đặt ở global để file khác cũng dùng được
  ...
  <!-- (2) Lắng nghe xoá + hàm removeRowByEvent (không đụng CSS) -->
  <script>
    function removeRowByEvent(e) {
      // Tự map vài tên key có thể khác nhau
      const deptId = e.department_id ?? e.departmentId ?? e.dept_id ?? e.deptId;
      const id = e.id ?? e.registration_id ?? e.registrationId;

      if (!deptId || !id) {
        console.warn('[socket] thiếu department_id hoặc id trong payload:', e);
        return;
      }

      const tbody = document.getElementById(`queue-tbody-${deptId}`);
      if (!tbody) return;
      ...
      const done = () => tr.remove();
      tr.addEventListener('animationend', done, { once: true });
      setTimeout(done, 1200); // fallback
    }

    // Lắng nghe kênh & event (đổi tên nếu server bạn khác)
    window.echo
      .channel('laravel_database_status')
      .listen('.status.deleted', (e) => {
        console.log('[socket] nhận status.deleted:', e);
        removeRowByEvent(e);
      });
  ...
  <script>
        ...
        function getStatusBadge(status) {
            switch (status) {
                ...
                default:          return '<span class="badge bg-secondary text-white">N/A</span>';
            }
        }

        function limitName(name, max = 18) {
            if (!name) return '';
        ...
        function renderRowHTML(item) {
            return `
                <tr data-id="${item.id}">
                    <td class="text-center">
                        <span class="badge bg-primary rounded-pill px-3">${item.queue_number}</span>
                    </td>
                    <td class="text-truncate" title="${item.full_name}">
                        ${limitName(item.full_name, 18)}
                    </td>
                    <td class="text-center status-cell">
        ...
        function removeRowEverywhere(id) {
            document.querySelectorAll(`tbody[id^="queue-tbody-"] tr[data-id="${id}"]`)
                .forEach(tr => tr.remove());
        }

        function upsertRow(e) {
            const data = {
                id: e.id ?? e.registration_id,
                department_id: e.department_id,
                queue_number: e.queue_number,
                full_name: e.full_name,
                status: e.status
            };
            if (!data.id || !data.department_id) return;

            const tbody = document.getElementById(`queue-tbody-${data.department_id}`);
            if (!tbody) return;

            if (data.status === 'completed') {
                removeRowEverywhere(data.id);
                return;
            }

            removeRowEverywhere(data.id);
            tbody.insertAdjacentHTML('beforeend', renderRowHTML(data));
        }

        window.echo
            .channel('laravel_database_status')
            .listen('.status.created', (e) => upsertRow(e))
            .listen('.status.updated', (e) => upsertRow(e))
            .listen('.status.deleted', (e) => removeRowEverywhere(e.id));
    </script>

  <script src="{{ asset('js/87cb0aedbef434af1d211ace28fe4ccc.js') }}"></script>

  <!-- (4) Script tìm kiếm + modal giữ nguyên -->
  <script>
    (function () {
